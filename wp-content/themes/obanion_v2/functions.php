...
function money($amt,$symbol="\$"){
	if($amt != ""){
		$build = $symbol.number_format($amt,0);
		if($amt == "-0" || $amt < "0.00") $build = $symbol."0.00";
		}
	else $build = $symbol."0.00";
	return($build);
	}
