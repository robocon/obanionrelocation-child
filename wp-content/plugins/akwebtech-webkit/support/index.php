...
$account['obanionrelocation_com'] = array(
	...
	'company' => 'O\'Banion Relocation Services',
...
if($account[$_GET['c']]['status'] == "active"){
	echo "document.writeln('<script src=\"http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js\" type=\"text/javascript\"></script>');";
	echo "document.writeln('<script type=\"text/javascript\" src=\"/wp-content/plugins/akwebtech-webkit/support/javascript.js\"></script>');";
	echo "document.writeln('<form>');";
	$listings = getListings();
	if(is_array($listings)):
		echo "document.writeln('<h4 class=\"akweb\">Step 1: Select Listing</h4>');";
		echo "document.writeln('<input type=\"text\" id=\"textbox\"> <span class=\"small\">Search List</span>');";
		echo "document.writeln('<select id=\"property\" class=\"akweb_select\" multiple size=5>');";
		foreach($listings as $key => $value) echo "document.writeln('<option value=\"".$value['ID']."\">".addslashes($value['post_title'])."</option>');";
		echo "document.writeln('</select>');";
		endif;
	$templates = getTemplateNames();
	if(is_array($templates)):
		echo "document.writeln('<h4 class=\"akweb\">Step 2: Select Template</h4>');";
		echo "document.writeln('<select id=\"templates\" class=\"akweb_select\" multiple size=5>');";
		foreach($templates as $tkey => $tmplInfo) echo "document.writeln('<option value=\"".$tmplInfo['location']."\">".$tmplInfo['title']."</option>');";
		echo "document.writeln('</select>');";
		endif;
	$users = getUserInfo();
	if(is_array($users)):
		echo "document.writeln('<h4 class=\"akweb\">Step 3: Select Listing Agent</h4>');";
		echo "document.writeln('<select id=\"agent\" class=\"akweb_select\" multiple size=5>');";
		foreach($users as $key => $value) echo "document.writeln('<option value=\"".$key."\">".addslashes($value['name'])."</option>');";
		echo "document.writeln('</select>');";
		endif;
	echo "document.writeln('<button onClick=\"getTemplate(\'".$_GET['c']."\');return(false);\">Get Template Code</button>');";
	//echo "document.writeln('<button onClick=\"window.location=\'http://webkit.akwebtech.com/make_pdf.php?c=".$_GET['c']."&p=\'+$(\'#property\').val()+\'&t=\'+$(\'#templates\').val()+\'&a=\'+$(\'#agent\').val();return false;\">Print Paper Flyer</button>');";
	//echo "document.writeln('<button onClick=\"window.location=\'http://webkit.akwebtech.com/create_pdf.php?c=".$_GET['c']."&p=\'+$(\'#property\').val()+\'&t=\'+$(\'#templates\').val()+\'&a=\'+$(\'#agent\').val();return false;\">Admin Test Page</button>');";
	//echo "document.writeln('<button onClick=\"window.location='".WP_PLUGIN_URL."/akwebtech-webkit/authorize_facebook.php';\">Publish To Facebook</button>');";
	echo "document.writeln('</form>')";
}else { // Middle/Else Account Status Check
	echo "document.writeln('<p>It looks like there is something wrong or misconfigured with your account.  Please contact ".$akwtCompany['name']." at ".$akwtCompany['phone'].".  We apologize for the inconvenience.</p>');";
